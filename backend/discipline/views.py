...
from .serializers import (
    DisciplinaryActionSerializer, DisciplineTypeSerializer
)
...
class DisciplinaryActionAPIView(APIView):
    ...
    def get(self, request):
        ...
        serializer = DisciplinaryActionSerializer(actions, many=True)
...
class DisciplinaryActionDetailAPIView(APIView):
    ...
    def patch(self, request, pk):
        action = get_object_or_404(DisciplinaryAction, pk=pk)
        serializer = DisciplinaryActionSerializer(action, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    @extend_schema(
        summary="Delete a disciplinary action"
    )
...
class DisciplineTypeAPIView(APIView):
    ...
    def get(self, request):
        ...
        serializer = DisciplineTypeSerializer(types, many=True)
        return Response(serializer.data)
...
class DisciplineTypeDetailAPIView(APIView):
    ...
    @extend_schema(
        responses=DisciplineTypeSerializer,
        summary="Retrieve a discipline type by ID"
    )
    def get(self, request, pk):
