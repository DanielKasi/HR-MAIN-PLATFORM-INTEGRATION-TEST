...
class EmployeeAllowance(models.Model):
    ...
    calculation_method = models.CharField(max_length=20, choices=CALCULATION_CHOICES, default='fixed')
    ...
    def calculate_totals(self):
        ...
        for allowance in self.employee.allowances.filter(is_active=True):
            if (allowance.effective_from <= self.payroll_period.end_date and 
                (not allowance.effective_to or allowance.effective_to >= self.payroll_period.start_date)):
        ...
        for deduction in self.employee.deductions.filter(is_active=True):
            if (deduction.effective_from <= self.payroll_period.end_date and 
                (not deduction.effective_to or deduction.effective_to >= self.payroll_period.start_date)):
