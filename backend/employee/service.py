...
class EmployeeBranchService:
    ...
    def attach_employee_to_branch(employee, branch, is_default=False, created_by=None):
        ...
        try:
            ...
            logger.info(f"UserBranch {'created' if created else 'updated'}: {user_branch.id}")
            return user_branch
    ...
    def attach_employee_to_multiple_branches(employee, branches_data, created_by=None):
        ...
        try:
            logger.info(f"Attaching employee {employee.id} to {len(branches_data)} branches")
    ...
    def set_default_branch(employee, branch):
        ...
        try:
            logger.info(f"Setting branch {branch.id} as default for employee {employee.id}")
    ...
    def remove_employee_from_branch(employee, branch):
        ...
        try:
            ...
            if was_default:
                remaining_branches = UserBranch.objects.filter(user=employee.user).first()
                if remaining_branches:
                    remaining_branches.is_default = True
                    remaining_branches.save()
                    logger.info(f"Set branch {remaining_branches.branch.id} as new default")
