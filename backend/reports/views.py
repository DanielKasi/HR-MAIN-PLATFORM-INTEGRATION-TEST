...
from .serializers import ReportChoicesSerializer, ReportGenerateInputSerializer, ReportGenerateQuerySerializer
    ...
    parameters=[
        {
            ...
            'schema': {'type': 'string', 'enum': list(build_reports_registry().keys())},  # Dynamic enum
        }
    ],
    responses={
        200: OpenApiResponse(
            ...
            examples=[
                OpenApiExample(
                    'Recruitment Example',
                    value={'report_types': ['candidates', 'interviews', 'onboardings']},
...
class ReportChoicesView(APIView):
    def get(self, request):
        ...
        if not app_name:
            return Response({'error': 'Missing "app" parameter'}, status=status.HTTP_400_BAD_REQUEST)
...
#             'required': True,
#             'description': 'App name (e.g., "recruitment")',
#             'schema': {'type': 'string', 'enum': list(build_reports_registry().keys())},
#         },
#         {
#             'name': 'report_type',
#             'in': 'query',
...
#     responses={
#         200: OpenApiResponse(
#             description="Generated report file (Excel: .xlsx, PDF: .pdf).",
#             media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',  # Defaults to Excel; Swagger handles
#         ),
...
class ReportGenerateView(APIView):
    def get(self, request):
        serializer = ReportGenerateQuerySerializer(data=request.query_params)
    ...
    def generate_excel(self, data, filename_prefix):
        ...
        response['Content-Disposition'] = f'attachment; filename={filename_prefix}_report.xlsx'
        return response
    ...
    def generate_pdf(self, data, filename_prefix):
        ...
        p.drawString(100, height - 50, f'{filename_prefix.replace("_", " ").title()} Report')
        ...
        response['Content-Disposition'] = f'attachment; filename={filename_prefix}_report.pdf'
        return response    