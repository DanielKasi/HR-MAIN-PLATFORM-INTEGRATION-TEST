...
class InstitutionTaxRule(BaseApprovableModel):
    ...
    def clean(self):
        ...
        if count > 1:
            raise ValidationError({"error": "Only one of tax_rule_percentage, tax_rule_fixed_amount, or tax_rule_formula can be specified."})
        if count == 0:
            raise ValidationError({"error": "At least one of tax_rule_percentage, tax_rule_fixed_amount, or tax_rule_formula must be specified."})
        if self.tax_rule_formula and not self.taxable_income_source:
            raise ValidationError({"error": "taxable_income_source must be specified when using a formula."})
