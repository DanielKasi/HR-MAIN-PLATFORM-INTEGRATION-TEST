...
class InstitutionSpotCheckSettingDetailView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: SpotCheckSerializers.InstitutionSpotCheckSettingSerializer, 404: "institution Setting not found"},
...
class BranchSpotCheckSettingDetailView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: SpotCheckSerializers.BranchSpotCheckSettingSerializer, 404: "Branch Setting not found"},
...
class EmployeeSpotCheckSettingDetailView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: SpotCheckSerializers.EmployeeSpotCheckSettingSerializer, 404: "Employee Spot check Setting not found"},
...
class EmployeeSpotCheckCreateView(APIView):
    @extend_schema(
        ...
        responses={201: SpotCheckSerializers.EmployeeSpotCheckCreateSerializer, 400: "Bad Request"},
    ...
    def post(self, request):
        ...
        serializer = SpotCheckSerializers.EmployeeSpotCheckCreateSerializer(data=request.data)
        if serializer.is_valid():
            spotcheck = serializer.save()
            spotcheck_status, _ =SpotCheckModels.SpotCheckStatus.objects.get_or_create(status_name="SENT")

            spotcheck.spotcheck_time = datetime.now()
            spotcheck.status = spotcheck_status
            spotcheck.initiated_by = "User"
...
class EmployeeSpotCheckUpdateView(APIView):
    ...
    def patch(self, request, spotcheck_id):
        ...
        serializer = SpotCheckSerializers.EmployeeSpotCheckSerializer(instance=setting, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class EmployeeSpotCheckInView(APIView):
    ...
    def patch(self, request, spotcheck_id):
        ...
        if serializer.is_valid():
            ...
            valid_status, _ = SpotCheckModels.SpotCheckStatus.objects.get_or_create("VALID", "VALID")
            invalid_status, _ = SpotCheckModels.SpotCheckStatus.objects.get_or_create("INVALID", "INVALID")
