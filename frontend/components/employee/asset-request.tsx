...
const AssetRequests: React.FC<AssetRequestsProps> = ({
	...
	const fetchFirstPage = useCallback(async () => {
		...
		try {
			return await assetsAPI.getPaginatedAssetRequests({
				...
				status: isEmployeeView ? undefined : statusFilter !== "all" ? statusFilter : undefined,
			});
		} catch (error: any) {
			if (
	...
	const StatusBadge = useCallback(
		({ status, isEmployeeView }: { status: string; isEmployeeView: boolean }) => (
												...
												{filteredResults.map((request) => (
													...
													>
														...
														<TableCell className={isEmployeeView ? "py-3 px-4" : ""}>
															<StatusBadge
																status={request.asset_request_status}
