  ...
  return (
    <div className="relative">
      ...
      {hoveredDay && isTooltipVisible && (
        <div
          ref={tooltipRef}
          className="absolute z-50 bg-white border border-slate-200 rounded-lg shadow-lg p-3 min-w-48 max-w-64 transition-all duration-200 ease-out"
          style={{
              ...
              <div className="space-y-2">
                ...
                {events.slice(0, 3).map((event) => (
                  <div key={event.id} className="flex items-start gap-2 text-xs">
                    <div className="flex-shrink-0 mt-0.5">{getEventModeIcon(event.event_mode)}</div>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-slate-900 truncate">{event.title}</div>
                      {event.description && (
                        <div className="text-slate-600 line-clamp-2 mt-1">{event.description}</div>
                      )}
                    </div>
                  </div>
                ))}
