...
import { formatTime, hasPermission } from "@/lib/helpers";
...
interface WorkingDaysManagerProps {
	scope:
		...
		| { type: "employee"; employeeWorkingDays: IEmployeeWorkingDays | null };
	...
	onEmployeeDaysUpdate?: (days: number[]) => Promise<void>;
	isSaving?: boolean;
}
...
export function WorkingDaysManager({
	...
	const [selectedEmployeeDays, setSelectedEmployeeDays] = useState<number[]>([]);
	...
	useEffect(() => {
		if (scope.type === "institution" && scope.institutionWorkingDays?.institution_days) {
			...
			setSelectedEmployeeDays(scope.employeeWorkingDays?.days || []);
		}
	}, [scope]);
	...
	const handleRemoveInstitutionDay = async (dayId: number) => {
		...
		try {
			...
			if (dayId === pendingAddDayId || dayId === editingDayId) {
				cancelEditing();
			}
		} catch (error) {
	...
	const handleRemoveBranchDay = async (dayId: number) => {
		...
		try {
			...
			showErrorToast({ error, defaultMessage: "Failed to remove day" });
		}
	};
	...
	const handleAddEmployeeDay = async (
		dayId: number,
		opening_time: string | null,
		closing_time: string | null,
	) => {
		...
		if (validateTimes(opening_time, closing_time)) {
			...
			// 	closing_time,
			// };
			const proposedDays = [...selectedEmployeeDays, dayId];

			try {
				if (onEmployeeDaysUpdate) {
					await onEmployeeDaysUpdate(proposedDays);
					setSelectedEmployeeDays(proposedDays);
				}
				setPendingAddDayId(null);
				setCurrentDayStartTime(null);
				setCurrentDayEndTime(null);
	...
	const handleRemoveEmployeeDay = async (dayId: number) => {
		if (scope.type !== "employee") {
			return;
		}
		const updatedDays = selectedEmployeeDays.filter((day) => day !== dayId);

		try {
			if (onEmployeeDaysUpdate) await onEmployeeDaysUpdate(updatedDays);
			setSelectedEmployeeDays(updatedDays);
			if (dayId === pendingAddDayId || dayId === editingDayId) {
				cancelEditing();
			}
		} catch (error) {
	...
	// const handleUpdateEmployeeDay = async (
	// 	dayId: number,
	// 	opening_time: string | null,
	// 	closing_time: string | null,
	// ) => {
	// 	if (validateTimes(opening_time, closing_time)) {
	// 		const updatedDays = selectedEmployeeDays.map((d) =>
	// 			d.day_id === dayId ? { ...d, day_type, opening_time, closing_time } : d,
	// 		);
	...
	// 		setCurrentDayEndTime(null);
	// 		} catch (error) {
	// 			showErrorToast({error, defaultMessage:"Failed to update day"})
	// 		}
	// 	}
	// };
	...
	return (
		<div className="space-y-6">
			<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:flex lg:flex-wrap xl:grid xl:grid-cols-7 gap-4">
				{sortedDays.map((day) => {
					const isSelected =
					...
					return (
						<div key={day.id} className="relative lg:w-48 xl:w-full">
							<div
							...
							>
								<div className="mb-2 flex justify-center">
									{!isSelected ? (
										...
										<>
											{isSaving && day.id === pendingAddDayId ? (
									...
									onClick={async (e) => {
										...
										scope.type === "institution"
											...
											: scope.type === "branch"
												...
												: hasPermission(PERMISSION_CODES.CAN_UPDATE_EMPLOYEE_WORKING_DAYS) &&
													(await handleRemoveEmployeeDay(day.id));
									}}
