...
export const ApprovalTimeline: React.FC<ApprovalTimelineProps> = ({ approval, approvals, className }) => {
    ...
    <div className={className}>
      {items.map((appr) => (
        <div key={appr.id} className="mb-4">
          <div className="text-sm font-semibold mb-2">{appr.document.description || `Approval #${appr.public_id}`}</div>
          <ol className="relative border-s border-gray-200 ml-2">
            {appr.tasks.map((t) => (
              <li key={t.id} className="mb-4 ms-4">
                ...
                <time className="mb-1 text-xs leading-none text-gray-500">Level {t.level.level}</time>
                <div className="text-sm font-medium">{t.level.name || `Level ${t.level.level}`}</div>
