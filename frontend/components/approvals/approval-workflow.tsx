...
import { mockApprovals } from "@/constants";
...
export function ApprovalWorkflow({
    ...
    instance_approval_status = "under_creation";
    ...
    useEffect(() => {
        const hasPendingTasks = approvals.some((approval) =>
            approval.tasks.some((task) => task.status === "pending"),
        );

        if (!hasPendingTasks && approvals.length > 0 && onRefresh) {
                ...
                <div key={approval.id} className="border rounded-lg p-4 space-y-4">
                    <div className="flex items-center justify-between border-b pb-3">
                        <div className="grid grid-cols-1 gap-3">
                            <Badge className={`${getActionColor(approval.action.name)} flex items-center gap-1`}>
                                {getActionIcon(approval.action.name)}
                                <span>
                                    This is a{" "}
                                    <b className="uppercase mx-1">
                                        {approval.action.name.charAt(0).toUpperCase() + approval.action.name.slice(1)}
                                    </b>{" "}
                                    operation{" "}
                                </span>
                            </Badge>
