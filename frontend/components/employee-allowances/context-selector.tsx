			...
			switch (selectedContext) {
				...
				case "department":
					...
					data = departments.map((dept: IDepartment) => ({
						...
						description:
							dept.description ||
							`Department in ${dept.institution_details?.institution_name || "Institution"}`,
					}));
					break;
				...
				case "job_position":
					...
					data = positions.map((pos: IJobPosition) => ({
						...
						name: pos.name,
						description:
							pos.description || `Position in ${pos.department_details?.name || "Department"}`,
					}));
					break;
		...
		<div className="space-y-4">
			...
			<div className="space-y-2">
				...
				<p className="text-xs text-gray-500">
					Choose whether this allowance applies to specific employees, entire departments, or job
					positions
				</p>
			</div>
			...
			{selectedContext && (
				<div className="space-y-2">
					...
					<p className="text-xs text-gray-500">
						Click on items to select/deselect them. You can select multiple{" "}
						{getContextLabel(selectedContext).toLowerCase()}.
					</p>
				</div>
			)}
		</div>
