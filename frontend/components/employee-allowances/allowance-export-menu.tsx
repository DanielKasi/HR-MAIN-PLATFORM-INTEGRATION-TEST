...
export function AllowanceExportMenu({
	allowances,
	getCalculatedAmount,
	disabled = false,
}: AllowanceExportMenuProps) {
	const exportToCSV = () => {
		const csvContent = [
			[
				"Employee",
	...
	const exportToExcel = () => {
		try {
			const excelData = allowances.map((allowance) => ({
				...
				"Calculation Method":
					allowance.calculation_method === "fixed" ? "Fixed Amount" : "Percentage",
				"Fixed Amount":
					allowance.calculation_method === "fixed" ? Number.parseFloat(allowance.amount) : "",
				Percentage:
					allowance.calculation_method === "percentage"
						? Number.parseFloat(allowance.percentage)
						: "",
				...
				"Effective To": allowance.effective_to
					? format(new Date(allowance.effective_to), "yyyy-MM-dd")
					: "",
