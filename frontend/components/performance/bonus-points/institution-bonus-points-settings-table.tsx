...
export function InstitutionBonusPointSettingsTable() {
  ...
  const [bonusPointSettingToDelete, setBonusPointSettingToDelete] = useState<IBonusPointSettings | null>(null);
  const [editingBonusPointSetting, setEditingBonusPointSetting] = useState<IBonusPointSettings | undefined>();
  ...
  const columns: ColumnDef<IBonusPointSettings>[] = [
    ...
    },
    {
      key: "condition",
      header: "Condition",
      cell: (setting) => `${setting.condition_field} ${setting.condition_operator} ${setting.condition_value}`,
    },
    {
      key: "actions",
      header: "Actions",
    ...
    <div className="overflow-x-auto w-full">
      ...
      <PaginatedTable<IBonusPointSettings>
        fetchFirstPage={async () =>
          await BONUS_POINT_SETTINGS_API.getPaginated({ page: 1 })
        }
