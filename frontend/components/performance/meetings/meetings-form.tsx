...
export function MeetingForm({ initialData, onSubmit, onCancel, isLoading }: MeetingFormProps) {
	...
	const validateForm = () => {
		...
		// Location validation (only for physical meetings)
		if (formData.mode === "physical" && !formData.location) {
			newErrors.location = "Location is required for physical meetings";
		}

		// Online link validation (optional, but must be valid if provided)
		if (formData.online_link && !formData.online_link.startsWith("http")) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const meetingData: IMeetingFormData = {
			...
			location: formData.mode === "physical" ? formData.location : undefined,
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
				{/* Location (only for physical meetings) */}
				...
				{/* Online Link (optional for online/hybrid) */}
			...
			<div className="space-y-2 md:col-span-2">
				...
				<Textarea
					...
					disabled={isLoading}
					className={cn("min-h-[100px] rounded-xl resize-none", errors.agenda && "border-red-500")}
					rows={3}
				/>
			...
			{formData.is_recurring && (
				<div className="space-y-2">
					<Label htmlFor="recurrence_rule" className="text-sm font-medium text-slate-700">
						Recurrence Rule <span className="text-red-500">*</span>
					</Label>
					<Input
						id="recurrence_rule"
						type="text"
					...
					{errors.recurrence_rule && (
						<p className="text-sm text-red-600">{errors.recurrence_rule}</p>
					)}
				</div>
			)}
			...
			<div className="flex justify-end gap-3 pt-4 border-t">
				...
				<Button type="submit" disabled={isLoading} className="px-8 rounded-full">
