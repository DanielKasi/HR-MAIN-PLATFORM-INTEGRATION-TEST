...
import { ChevronDown, ChevronRight, Settings, User, LogOut, ChevronLeft, Menu, X } from "lucide-react"
...
import { selectAccessToken, selectSelectedInstitution, selectUser, selectUserLoading } from "@/store/auth/selectors"
import {
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (userData && userData.id && selectedInstitution && selectedInstitution.institution_owner_id) {
      ...
      if (!role && Array.isArray(userData.roles) && userData.roles.length > 0) {
        ...
        if (matchingRole) role = matchingRole.name.charAt(0).toUpperCase() + matchingRole.name.slice(1).toLowerCase()
      }
  ...
  const updateThemeColors = (hexColor: string) => {
    ...
    try {
      ...
      document.documentElement.style.setProperty("--primary-hover", `${hue} ${saturation}% ${darkerL}%`)
      ...
      document.documentElement.style.setProperty("--sidebar-hover", `${hue} ${lighterS}% ${lighterL}%`)
      if (document.documentElement.classList.contains("dark")) {
        ...
        document.documentElement.style.setProperty("--sidebar-hover", `${hue} ${saturation}% ${darkModeHoverL}%`)
      } else {
    ...
    if (userData) {
      try {
        ...
        if (user.fullname) {
          ...
          setUserInitials(
            nameParts.length > 1 ? `${nameParts[0][0]}${nameParts[1][0]}`.toUpperCase() : nameParts[0][0].toUpperCase(),
          )
        }
      } catch (error) {
  ...
  const renderNavigationItem = (item: NavItem, isMobileView = false) => {
    const isActive = item.submenu ? item.submenu.some((sub) => pathname === sub.href) : pathname === item.href
    ...
    return (
      <div key={item.title} className="w-full py-1">
        ...
        >
          ...
          {isSideBarOpen ? item.submenu && (isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />): <></>}
          
        </Button>
        {item.submenu && isExpanded && (
          <div className="ml-6 flex flex-col mt-2 border-l-2 border-primary/20 bg-gray-200/20">
            {isSideBarOpen && item.submenu.map((sub) => (
              <Button
                key={sub.href}
                variant="ghost"
                className={`w-full !rounded-none !text-left flex items-center px-2 !py-4 text-sm text-gray-600 hover:bg-primary/80 ${
                  pathname === sub.href ? "bg-primary/80  text-gray-100" : "bg-gray-200/20  hover:bg-primary/60"
                }`}
                onClick={() => {
        ...
        >
          <div className="p-4 border-b border-gray-100 min-h-16 h-20 max-h-20 flex items-center">
            <div className="flex items-center gap-3">
              ...
              {isSideBarOpen && <span className="font-bold text-[var(--sidebar-foreground)]">{InstitutionName}</span>}
            </div>
          </div>
