...
import { Briefcase, ArrowLeft, Check, Upload, X, FileText, Loader2, Users, DollarSign, AlertCircle, Search, ChevronDown } from "lucide-react"
...
const VirtualizedEmployeeList: React.FC<{
  ...
  onEmployeeToggle: (employee: Employee, checked: boolean) => void
  searchTerm: string
        ...
        <div style={{ transform: `translateY(${offsetY}px)` }}>
          {visibleEmployees.map((employee, index) => {
            ...
            return (
              <div
                key={employee.id}
                className={`flex items-center space-x-3 p-3 border-b hover:bg-muted/50 transition-colors cursor-pointer ${
...
const EmployeeSelectionModal: React.FC<{
  ...
  const handleEmployeeToggle = useCallback((employee: Employee, checked: boolean) => {
  ...
  const handleDepartmentSelect = useCallback((departmentEmployees: Employee[], select: boolean) => {
    setSelectedEmployees(prev => {
      const newSet = new Set(prev)
