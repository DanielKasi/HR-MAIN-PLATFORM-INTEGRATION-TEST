  ...
  const handleDeleteApplication = async (id: string | number) => {
    ...
    try {
      ...
      if (success) {
        ...
        toast.error("Failed to delete leave application. Only pending applications can be deleted.")
        await refreshApplications()
      }
    } catch (error: any) {
    ...
    return (
      <>
        <div className="space-y-2">
          <Label htmlFor={isEdit ? "edit-start_date" : "start_date"} className="text-sm font-medium">
            Start Date *
          </Label>
          <Input
            ...
            min={new Date(Date.now() + (selectedPolicy?.min_notice_days || 10) * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}
          />
          {selectedPolicy && (
                ...
                <TableRow
                  ...
                  <TableCell>
                    <span className="text-sm text-gray-900">
                      {DURATION_TYPES.find((d) => d.value === application.duration_type)?.label || application.duration_type}
                    </span>
                  </TableCell>
                  <TableCell>
                    <Badge
          ...
          {filteredApplications.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {statusFilter === "all" ? "No leave applications have been submitted yet." : `No ${statusFilter} applications found.`}
              </p>
            </div>
          )}
        </div>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">
                      {DURATION_TYPES.find((d) => d.value === viewingApplication.duration_type)?.label || viewingApplication.duration_type}
                    </p>
                  </div>
                  <div>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
            ...
            <Button
              ...
              disabled={isSubmitting || validateLeaveApplication().filter(v => v.type === 'error').length > 0}
            >
              {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
