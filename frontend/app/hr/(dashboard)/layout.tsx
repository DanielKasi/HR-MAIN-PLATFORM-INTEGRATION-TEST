...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  console.log("\n\n\n HR Module Path : ", process.env.NEXT_PUBLIC_HR_MODULE_PATH);
  
  // Get the HR module path
  const hrModulePath = process.env.NEXT_PUBLIC_HR_MODULE_PATH || '';
  
  // Create the final href (don't modify the original item)
  const finalHref = hrModulePath + item.href;
  ...
  if (item.submenu && item.submenu.length > 0) {
    ...
    isActive = item.submenu.some(sub => {
      const subFinalHref = hrModulePath + sub.href;
      return pathname === subFinalHref;
    });
  } else {
      ...
      <DropdownMenu key={item.title}>
        ...
        <DropdownMenuContent align="start" className="w-48">
          {item.submenu.map((option, index) => {
            // Add HR module path to each submenu item
            const subFinalHref = hrModulePath + option.href;
            ...
            return (
              <div key={option.href}>
                <DropdownMenuItem 
                  ...
                  onClick={() => {
                    console.log("Navigating to:", subFinalHref);
                    router.push(subFinalHref);
                  }}
                >
                  {option.title}
                </DropdownMenuItem>
