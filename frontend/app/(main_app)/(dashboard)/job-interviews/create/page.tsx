...
export default function CreateInterviewPage() {
  ...
  const filteredInterviewStages = useMemo(
    () =>
      selectedJobPosition
        ? interviewStages.filter(
            (stage) => stage.job_position_advert === Number(selectedJobPosition),
          )
        : [],
  ...
  const fetchInitialData = async () => {
    ...
    try {
      const [fetchedApplicationsResponse, fetchedStagesResponse, fetchedInterviewsResponse] =
        await Promise.all([
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <PaginatedSearchableSelect<any, {search?: string; page?: number}>
                  ...
                  getItemLabel={(position) =>
                    position.job_position_details?.name || `Position ${position.id}`
                  }
                      ...
                      <div className="space-y-3">
                        {filteredGroupedApplications[
                          ...
                          <div className="text-center py-4 text-muted-foreground">
                            ...
                            <p className="text-sm">
                              No applicants found for this position or all applicants have already
                              been scheduled
                            </p>
                          </div>
                        )}
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t">
                ...
                <Button
                  type="submit"
                  disabled={
                    isSubmitting || (selectedApplications.length === 0 && !selectedJobPosition)
                  }
