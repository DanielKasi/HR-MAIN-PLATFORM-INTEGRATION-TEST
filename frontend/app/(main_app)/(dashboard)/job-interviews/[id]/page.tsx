...
export default function InterviewViewPage() {
	...
	return (
		<div className="w-full h-full p-6 space-y-6 rounded-lg bg-white shadow-sm">
			...
			<div
				className={` gap-6 ${interview?.approval_status !== "active" && interview?.approvals?.length ? "!grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-3" : ""}`}
			>
				...
				<div
					className={`${interview?.approval_status !== "active" && interview?.approvals?.length ? "lg:col-span-2 xl:col-span-3 order-2 lg:order-1" : ""}`}
				>
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
						{/* Main Content */}
