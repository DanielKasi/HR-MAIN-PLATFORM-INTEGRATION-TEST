			...
			<div className="flex items-center justify-center min-h-screen">
				<div className="text-center">
					...
					<p className="text-gray-600 mb-4">
						The allocation you're looking for doesn't exist or has been removed.
					</p>
									...
									allocation.tasks.map((task: any, index: any) => (
										<div key={task.id} className="relative">
											<div className="flex items-start gap-3 lg:gap-4">
												<div
													className={`w-6 h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center flex-shrink-0 border-2 ${
												...
												<div className="flex flex-col bg-gray-50 rounded-lg p-3 lg:p-4 flex-1">
													...
													{task.status === "completed" && task.comments && (
														<p className="text-xs text-gray-500 mt-1">{task.comments}</p>
													)}
													{task.status === "pending" && (
											...
											{index < allocation.tasks.length - 1 && (
												<div
													className="absolute left-3 lg:left-4 top-6 lg:top-8 w-0.5 h-6 lg:h-8 ml-1"
													style={{
														backgroundImage:
															"repeating-linear-gradient(0deg, transparent, transparent 2px, #d1d5db 2px, #d1d5db 4px)",
													}}
