...
import type { TicketCategory } from "@/types/help-desk.types";
import { showErrorToast, showSuccessToast } from "@/lib/utils";
import { ConfirmationDialog } from "@/components/confirmation-dialog";
import FixedLoader from "@/components/fixed-loader";
import { Icon } from "@iconify/react";
import TicketCategoryCreateEditDialog from "./_components/ticket-category-create-edit-dialog";
...
export default function TicketCategoriesPage() {
	...
	const handleDelete = async () => {
		...
		try {
			setDeleting(true);
			await TICKET_CATEGORIES_API.delete({ categoryId: categoryToDelete.id });
			showSuccessToast("Ticket Category deleted successfully!");
			if (tableRefreshRef.current) tableRefreshRef.current();
		} catch (err) {
			showErrorToast({ error: err, defaultMessage: "Failed to delete Ticket category" });
		} finally {
			setDeleting(false);
	...
	const openDetails = (category: TicketCategory) => {
		setSelectedCategory(category);
	...
	const renderDetailsContent = (category: TicketCategory) => (
		...
		<div className="p-6 space-y-6 bg-white rounded-lg min-h-screen">
			<div className="flex justify-between items-center">
				...
				<div className="flex items-center justify-end gap-4">
					...
					>
						<Plus className="h-4 w-4 mr-2" /> Create Ticket Category
					</Button>
			...
			<PaginatedTable<TicketCategory>
				fetchFirstPage={async () => {
					if (!currentInstitution) throw new Error("No institution selected");
					return await TICKET_CATEGORIES_API.getPaginated({
						page: 1,
						search: searchTerm || undefined,
						ordering,
					});
				}}
				fetchFromUrl={TICKET_CATEGORIES_API.getPaginatedFromUrl}
				...
				emptyState={
					<div className="text-center py-12">
						<p className="text-muted-foreground mb-4">No ticket categories found</p>
					</div>
				}
			/>
			...
			{selectedCategory && (
				<ApprovableDialog
					...
					title={`Ticket Category Details: ${selectedCategory.name}`}
					description="View the details for this ticket category."
