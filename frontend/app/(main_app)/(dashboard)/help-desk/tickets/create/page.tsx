...
export default function TicketCreatePage() {
	...
	const handleAddComment = () => {
		if (newComment.trim()) {
			setFormData((prev) => ({
				...prev,
				new_comments: [newComment], // Only one comment allowed
			}));
			setNewComment("");
		}
	};
	...
	const handleCreate = async () => {
		...
		try {
			...
			formDataToSend.append("status", formData.status); // Explicitly send OPEN
			...
			showErrorToast({ error: err, defaultMessage: "Failed to create ticket" });
		} finally {
			setSaving(false);
		}
	};
			...
			<div className="space-y-4">
				...
				<div className="space-y-2 w-full lg:w-1/2">
					...
					<Textarea
						...
						rows={3}
						disabled={formData.new_comments && formData.new_comments?.length > 0}
					/>
					{formData.new_comments && formData.new_comments?.length > 0 && (
						<div className="space-y-2">
							{formData.new_comments.map((comment, index) => (
								<div key={index} className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl">
