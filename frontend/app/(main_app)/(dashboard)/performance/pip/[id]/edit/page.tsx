...
import PIPSupportResourceSearchableSelect from "../support-resource-searchable-select";
...
export default function PerformanceImprovementPlanEditPage() {
	...
	useEffect(() => {
		...
		const fetchPlan = async () => {
			try {
				setLoading(true);
				const plan = await PERFORMANCE_IMPROVEMENT_PLAN_API.getById({ pipId: planId });
				setFormData({
					employee: plan.employee?.id || 0,
					start_date: plan.start_date || "",
					end_date: plan.end_date || "",
					issues: (plan.issues as IPerformanceConcern[])?.map((issue) => issue.id) || [],
					support_resources:
						(plan.support_resources as IPIPSupportResource[])?.map((res) => res.id) || [],
					progress_notes: plan.progress_notes || "",
					consequences: plan.consequences || "",
					status: plan.status || "draft",
					final_review_date: plan.final_review_date || "",
					outcome: plan.outcome || "",
					objectives: (plan.objectives as IObjective[])?.map((obj) => obj.id) || [],
					document_template: plan.document_template,
				});
			} catch (err) {
				showErrorToast({
					error: err,
	...
	const handleUpdate = async () => {
		...
		try {
			setSaving(true);
			await PERFORMANCE_IMPROVEMENT_PLAN_API.update({ pipId: planId, data: formData });
			showSuccessToast("Performance improvement plan updated successfully!");
			router.push(`/admin/performance/performance-improvement-plans/${planId}`);
		} catch (err) {
			showErrorToast({
				error: err,
				defaultMessage: "Failed to update performance improvement plan",
			});
		} finally {
			setSaving(false);
		}
		...
		<div className="p-6 space-y-6 bg-white rounded-lg min-h-screen">
			<div className="flex justify-between items-center">
				<div className="flex items-center justify-start gap-4">
					<Link href={`/admin/performance/performance-improvement-plans/${planId}`}>
