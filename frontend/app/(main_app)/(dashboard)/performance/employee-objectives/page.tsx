...
import { EMPLOYEE_OBJECTIVES_API } from "@/lib/utils"
...
export default function EmployeeObjectivesPage() {
    const [employeeObjectives, setEmployeeObjectives] = useState<IEmployeeObjectives[]>([])
    const [loading, setLoading] = useState(true)
    const [modalOpen, setModalOpen] = useState(false)
    const [editingEmployeeObjective, setEditingEmployeeObjective] = useState<IEmployeeObjectives | undefined>()
    ...
    const handleEdit = (employeeObjective: IEmployeeObjectives) => {
        setEditingEmployeeObjective(employeeObjective)
        setModalOpen(true)
    }

    const handleSubmit = async (data: IEmployeeObjectivesFormData) => {
        setSubmitting(true)
        try {
    ...
    const handleDelete = async (employeeObjective: IEmployeeObjectives) => {
        const employeeName = employeeObjective.employee.user?.fullname || "Unknown"
        const objectiveName = employeeObjective.objective.name

        if (!confirm(`Are you sure you want to remove "${objectiveName}" from ${employeeName}?`)) return
        ...
        try {
            ...
            toast.error(error.message || "Failed to remove objective assignment")
        }
    }
    ...
    const atRisk = employeeObjectives.filter((eo) => eo.status === "at_risk" || eo.status === "behind").length
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-600 text-lg">Assign and track individual employee objectives</p>
                        </div>
                    </div>
                </div>
