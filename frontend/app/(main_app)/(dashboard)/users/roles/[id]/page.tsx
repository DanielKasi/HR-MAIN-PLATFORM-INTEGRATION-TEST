...
export default function RoleDetailsPage() {
  ...
  return (
    <div className="p-4 max-w-full bg-white-50 min-h-screen bg-white rounded-xl">
      ...
      <div className={` gap-6 ${(role?.approval_status !== "active" && role?.approvals?.length) ? "!grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-3" : ""}`}>
        ...
        <div className={`${(role?.approval_status !== "active" && role?.approvals?.length) ? "lg:col-span-2 xl:col-span-3 order-2 lg:order-1" : ""}`}>

          <div className="flex items-center mb-6">
            <Button variant="ghost" className="rounded-full aspect-square" onClick={() => router.push("/users/roles")}>
              <ArrowLeft />
            </Button>
            <h1 className="text-2xl font-bold">Role Details</h1>
          </div>
