...
export default function TaskPriorityPage() {
	...
	const [editingStatus, setEditingStatus] = useState<ITaskPriority | null>(null);

	const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
	const [statusToDelete, setStatusToDelete] = useState<ITaskPriority | null>(null);
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 gap-4">
				<div className="space-y-1">
					<div className="flex items-center gap-2">
						<h1 className="text-xl sm:text-2xl font-bold tracking-tight sm:ml-4">Task Priority</h1>
					</div>
					<p className="text-xs sm:text-sm text-muted-foreground sm:ml-4">
						Manage task priority types for your organization
					</p>
				</div>
				<ProtectedComponent permissionCode={PERMISSION_CODES.CAN_CREATE_TASK_PRIORITY}>
			...
			<PaginatedTable<ITaskPriority>
				...
				columns={[
					...
					},
					{
						...
						className: "w-[50px]",
						cell: (status) => {
							const canView = hasPermission(PERMISSION_CODES.CAN_VIEW_TASK_PRIORITY);
							const canEdit = hasPermission(PERMISSION_CODES.CAN_EDIT_TASK_PRIORITY);
							const canDelete = hasPermission(PERMISSION_CODES.CAN_DELETE_TASK_PRIORITY);
