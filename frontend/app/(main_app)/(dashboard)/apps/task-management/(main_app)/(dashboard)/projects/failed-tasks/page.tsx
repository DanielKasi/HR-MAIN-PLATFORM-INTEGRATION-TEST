...
import { showErrorToast, PROJECTS_TASKS_API, PROJECT_TASK_PRIORITY_API } from "@/lib/utils";
...
export default function FailedTasksPage() {
	...
	const [taskToDelete, setTaskToDelete] = useState<IProjectFailedTasks | null>(null);
	const [isDeleting, setIsDeleting] = useState(false);
	const [isAddTaskOpen, setIsAddTaskOpen] = useState(false);
	const [selectedTask, setSelectedTask] = useState<IProjectFailedTasks | null>(null);
	...
	const fetchFirstPage = useCallback(async () => {
		...
		try {
			...
			const res = await PROJECT_TASKS_STATUSES_API.getPaginatedProjectFailedTasks(params);
	...
	const fetchNextPage = useCallback(async () => {
		...
		setLoading(true);
		try {
			const res = await PROJECT_TASKS_STATUSES_API.getPaginatedProjectFailedTasksFromUrl({
				url: nextUrl,
			});
				...
				<div>
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
						<div className="space-y-1 sm:space-y-2">
							...
							<p className="text-slate-600 text-xs sm:text-sm">View all failed tasks</p>
						</div>
						{/* <ProtectedComponent
						...
						{tasks.map((task) => (
							...
							>
								...
								<div className="flex gap-2 mt-2 sm:mt-3">
									<Badge
										style={{
											backgroundColor: task.applied_project_task_status?.color_code
								...
								<div className="space-y-1 text-xs sm:text-sm text-gray-600 pt-3 sm:pt-4 border-t">
									...
									{!task.start_date && !task.end_date && !task.completion_date && (
										<div className="text-center text-gray-400">No dates set</div>
									)}
								</div>
							</div>
						))}
