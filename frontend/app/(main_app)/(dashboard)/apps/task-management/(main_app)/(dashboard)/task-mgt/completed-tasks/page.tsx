...
import { showErrorToast, TASKS_API, TASK_PRIORITY_API } from "@/lib/utils";
import { ConfirmationDialog } from "@/components/confirmation-dialog";

import { PERMISSION_CODES } from "@/constants";
import { ICompletedTasks, ITaskPriority } from "@/types/project.type";
...
export default function CompletedTasksPage() {
	...
	const [taskToDelete, setTaskToDelete] = useState<ICompletedTasks | null>(null);
	...
	const loadOptions = async () => {
		try {
			const [prioritiesRes] = await Promise.all([TASK_PRIORITY_API.getPaginatedTaskPriority({})]);
	...
	const fetchFirstPage = useCallback(async () => {
		...
		try {
			...
			const res = await TASKS_STATUSES_API.getPaginatedCompletedTasks(params);
	...
	const fetchNextPage = useCallback(async () => {
		...
		setLoading(true);
		try {
			const res = await TASKS_STATUSES_API.getPaginatedCompletedTasksFromUrl({ url: nextUrl });
