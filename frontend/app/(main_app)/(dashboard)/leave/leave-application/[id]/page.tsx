...
const LeaveApplicationDetailPage = () => {
	const [leaveApplication, setLeaveApplication] = useState<ILeaveRequest | null>(null);
	const [loading, setLoading] = useState(true);
	const router = useRouter();
	const params = useParams();
	const applicationId = params?.id as string;

	const fetchLeaveApplicationDetail = async () => {
		if (!applicationId) return;

		try {
			setLoading(true);
			const applicationData = await LeaveApplicationsAPI.getById(parseInt(applicationId));
			setLeaveApplication(applicationData);
		} catch (error) {
	...
	return (
		<div className="space-y-6 p-6 bg-white">
			...
			<ApprovableInstancePageLayout
				...
				onInstanceRefresh={fetchLeaveApplicationDetail}
			>
				<div className="grid gap-6 md:grid-cols-2">
