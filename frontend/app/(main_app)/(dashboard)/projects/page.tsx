...
export default function ProjectsPage() {
	...
	const columns: ColumnDef<IProject>[] = [
		...
		},
		{
			...
			cell: (project) => <Badge>{project.managers.length}</Badge>,
		},
		{
			key: "assignees",
			header: (
			...
			cell: (project) => <Badge>{project.assignees.length}</Badge>,
		},
		{
			key: "start_date",
			header: (
				...
				<ProtectedComponent permissionCode={PERMISSION_CODES.CAN_VIEW_PROJECTS}>
					<>
						<PaginatedTable<IProject>
							...
							query={searchTerm}
							onError={(err) =>
								showErrorToast({ error: err, defaultMessage: "Failed to fetch projects" })
							}
							className="space-y-4"
