...
const recalculateStageCandidateCounts = (
	...
	interviews.forEach((interview: IInterview) => {
		...
		if (
			!candidateLatestStage.has(candidateId) ||
...
const InterviewSchedulingDialog = ({
	...
	const validateScheduleForm = (): boolean => {
		...
		if (
			!scheduleData.interview_type ||
			!["online", "in_person"].includes(scheduleData.interview_type)
		) {
...
export default function UnifiedInterviewPipeline({ params }: UnifiedInterviewPipelineProps) {
	...
	const [selectedCandidateWithHistory, setSelectedCandidateWithHistory] =
		useState<CandidateWithHistory | null>(null);
	...
	const candidatesEligibleForOnboarding = filteredCandidates.filter(
		(candidate) =>
	...
	const candidatesEligibleForMoving = filteredCandidates.filter(
				...
				.map((stage, index) => {
					...
					const candidates: Candidate[] = stageInterviews.map((interview) => ({
						...
						job_position_advert:
							interview.job_position_application_details?.job_position_advert ||
							parseInt(resolvedParams.id),
						job_position_advert_job_details: interview.job_position_application_details
							?.job_position_advert_job_details || {
							name: "",
							description: "",
							job_posted_date: "",
					...
					const interviewerNames =
						stage.interviewers_details && Array.isArray(stage.interviewers_details)
					...
					return {
						...
						candidates: candidates ?? [], // Always provide an array
						color: colors.color,
	...
	const handleSelectCandidate = (candidateId: number, checked: boolean) => {
		const candidate = (
			viewMode === "current" ? filteredCandidates : filteredHistoryCandidates
	...
	const handleSelectAll = (checked: boolean) => {
		const candidatesToSelect =
			viewMode === "current"
				...
				: filteredHistoryCandidates.filter(canCandidateBeSelected);

		if (checked) {
			if (candidatesToSelect.length === 0) {
				if (viewMode === "current") {
					toast.info(
						"No candidates need feedback. All candidates have already been reviewed or onboarded.",
					);
				} else {
					toast.info(
	...
	return (
		<div className="p-6 space-y-6 bg-white rounded-xl">
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-gray-600">
							{jobPositionAdvert.job_position_details?.name || "Position"} - Complete interview
															...
															{candidatesEligibleForOnboarding.length === 0 &&
																...
																selectedCandidates.length > 0 && (
																	<div className="text-xs text-red-600 bg-red-50 px-3 py-1 rounded-full border border-red-200">
																		Please first provide feedback & rating, then schedule interviews
																		before onboarding
																	</div>
																)}
														</div>
											...
											<TableHeader>
												<TableRow>
													<TableHead className="w-12">
														<Checkbox
															checked={
																filteredHistoryCandidates.filter(canCandidateBeSelected).length >
																	0 &&
			...
			{processedStages.length > 0 && viewMode === "current" && (
				<div className="mt-6">
					{" "}
					{/* Added margin-top to push it down */}
					<Card>
						<CardContent className="p-6">
			...
			<InterviewStageDetailsDialog
				...
				stage={
					selectedStageForDetails
						? { ...selectedStageForDetails, candidates: selectedStageForDetails.candidates ?? [] }
						: null
				}
				candidate={null}
				size="lg"
			...
			<EditInterviewStageDialog
				...
				stage={
					selectedStageForEdit
						? { ...selectedStageForEdit, candidates: selectedStageForEdit.candidates ?? [] }
						: null
				}
