...
export default function SettingsPage() {
	...
	const handleUpdateProfile = async (e: React.FormEvent) => {
		...
		try {
			...
			const updatedUser = await usersAPI.update({ userId: currentUser.id, updateData });
			dispatch(setCurrentUser(updatedUser.user));
			toast.success("Profile updated successfully");
		} catch (error: any) {
			showErrorToast({ error, defaultMessage: "Failed to update profile" });
		} finally {
			setIsUpdating(false);
		}
	};
					...
					<div className="flex-1 lg:pl-8">
							...
							<div>
								...
								<form onSubmit={handleUpdateProfile} className="w-full max-w-md space-y-6">
									<TextInput
										id="name"
										label="Full Name"
										placeholder={currentUser ? currentUser.fullname : "Enter your full name"}
										value={name}
										onChange={setName}
						...
						{activeTab === "password" && (
							<div>
								...
								<form onSubmit={handleChangePassword} className="w-full max-w-md space-y-6">
									<PasswordInput
										id="old-password"
										label="Old Password"
